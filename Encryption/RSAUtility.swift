...
public class RSAUtility
{
	...
	func loadPublicKey(data: NSData) throws -> SecKey {
		if let cert = SecCertificateCreateWithData(kCFAllocatorDefault, data) {
			...
			if status == errSecSuccess, let trustRef = trust {
				if let key = SecTrustCopyPublicKey(trustRef) {
					return key
				}
				throw chip_genErrorRSA("Failed to retrieve public key from trust certificate")
			}
