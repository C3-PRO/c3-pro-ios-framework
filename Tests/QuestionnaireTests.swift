...
class QuestionnaireChoiceTests: XCTestCase {
	...
	func testContainedValueSet() {
		do {
			...
			let questionnaire = try bundle.fhir_bundledResource("Questionnaire-choices", subdirectory: "Questionnaire", type: Questionnaire.self)
			...
			controller.prepareQuestionnaire() { task, error in
				...
				XCTAssertEqual("example-question-choice", task?.identifier)
				...
				XCTAssertEqual(step1?.title, "A Simple Choice")
				...
				let step2 = task?.step!(withIdentifier: "choice-boolean-2") as? ConditionalQuestionStep
				XCTAssertNotNil(step2, "Should have found step 2b and made it a conditional question step")
